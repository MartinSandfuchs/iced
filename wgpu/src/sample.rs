...
enum Message {
    CopySample(gstreamer::Sample, wgpu::Buffer, wgpu::BufferWriteMapping),
    Exit,
}
    ...
    fn new(
        ...
        let jh = std::thread::spawn(move || {
            for msg in sample_receiver.iter() {
                match msg {
                    Message::CopySample(sample, buffer, mut write_mapping) => {
                        let extract_sample = || {
        ...
        let t_view_frame = t_frame.create_default_view();
...
impl Pipeline {
    pub fn new(device: &wgpu::Device) -> Self {
        ...
        let vs = include_bytes!("shader/sample.vert.spv");
        let vs_module = device.create_shader_module(
            &wgpu::read_spirv(std::io::Cursor::new(&vs[..]))
                .expect("Read image vertex shader as SPIR-V"),
        );

        let fs = include_bytes!("shader/sample.frag.spv");
        ...
        let bounds_buffer = device.create_buffer_with_data(
            bounds.as_bytes(),
    ...
    pub fn draw(
        ...
        for sample in samples {
            ...
            if Some(sample) != stream.cur_sample.as_ref() {
                // // We could possibly try to reuse the buffers, not sure if this m
                ...
                let fut = buffer
                    .map_write(0, (stream.width * stream.height * 4) as u64);
                device.poll(wgpu::Maintain::Wait);
                let write_mapping = futures::executor::block_on(fut).unwrap();
